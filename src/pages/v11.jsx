    ...
    <>
      ...
      >
        ...
        <section className="relative px-4 sm:px-6 lg:px-8 pb-12">
          <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.08)] border border-gray-100 p-5 lg:p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 lg:gap-3 items-end">
              {[
                { label: "Pickup location", icon: MapPin, type: "select", options: ["Ngurah Rai Airport (DPS)", "Ubud", "Seminyak", "Kuta", "Nusa Dua"] },
                { label: "Pick-up date & time", icon: Calendar, type: "datetime-local", value: "2026-05-18T10:00" },
                { label: "Return date & time", icon: Calendar, type: "datetime-local", value: "2026-05-21T10:00" },
                { label: "Vehicle type", icon: Car, type: "select", options: ["All vehicle types", "SUV", "MPV", "Sedan", "Convertible"] },
              ].map((f) => (
                <div key={f.label}>
                  <label className="block text-[11px] font-semibold text-gray-500 mb-2">{f.label}</label>
                  <div className="flex items-center gap-2 px-3 py-2.5 bg-gray-50 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
                    ...
                    {f.type === "select" ? (
                      ...
                      <input type={f.type} defaultValue={f.value} className="bg-transparent text-sm text-gray-700 outline-none w-full" />
                    )}
                  </div>
                </div>
              ))}
              <button
                className="inline-flex items-center justify-center gap-2 px-5 py-3 text-white text-sm font-bold rounded-lg transition-colors"
                style={{ backgroundColor: TEAL }}
                onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = TEAL_DARK)}
                onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = TEAL)}
              >
            ...
            {/* Trust strip with vertical separators */}
